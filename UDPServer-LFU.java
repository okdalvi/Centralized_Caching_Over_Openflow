...
sendData2 = sentence1.getBytes();
   ...
   public int getMax(LRU[] cachearray)
  {
	...
	return maxindex;  
      ...
      public int getMin(LRU[] cachearray)
  {
	  int i,min,minindex;
	  min=999;
	  minindex=0;
	  ...
	  for(i=0;i<3;i++)
	  {
	    if( min > cachearray[i].getCount())
	    {
	    	minindex=i;
                System.out.println("\nMin index="+i);
	        min=cachearray[i].getCount();
            }
...
int countval; 
  ...
  if((index > 3)&&(found==0))
  {
	  ...
	  cachearray[setindex].setURL(URL);
	 	cachearray[setindex].setCount(1);
      ...
      if(found==1)
      {
       setindex=findindex(URL,cachearray);
		...
		cachearray[setindex].setCount(maxval+1);
  ...
  cachearray[currentIndex].setCount(1);
  currentIndex++;
  for(int i=0;i<3;i++)
          {
            urlval=cachearray[i].getURL();
